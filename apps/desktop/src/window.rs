...
#[tauri::command]
pub fn cmd_run_click_through_latency_test(
    app: AppHandle,
    label: String,
    toggles: Option<u32>,
    ...
    let avg_ms = samples.iter().map(|sample| sample.elapsed_ms).sum::<f64>() / samples.len() as f64;
    ...
    println!(
        "[latency-test:{label}] target_ms={target_ms:.1} total_toggles={total_toggles} \
